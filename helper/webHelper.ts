...
async waitForSelector(
  ...
  try {
      ...
      console.log(`Selector '${selector}' reached state: '${state}' within ${timeout} ms.`);
  } catch (error) {
      console.error(`Failed to reach state '${state}' for selector '${selector}' within ${timeout} ms.`, error);
      throw error;
  }
}
...
async  deleteAllInstances(): Promise<void> {
  ...
  const elements = this.page.frameLocator("iframe").locator(`[data-testid="MoreVertIcon"]`).nth(1)  
  const deleteBtn = this.page.frameLocator("iframe").locator(`(//li[contains(@class,'MuiButtonBase-root MuiMenuItem-root')])[6]`).nth(0)
  const deleteConfirmBtn = this.page.frameLocator("iframe").locator(`//button[contains(text(), "Delete")]`).nth(0)
